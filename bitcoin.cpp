...
class CNode {
  ...
  void PushVersion() {
    ...
    string ver = "/ultracoin-seeder:0.01/";
...
public:
  CNode(const CService& ip, vector<CAddress>& vAddrIn) : you(ip), nHeaderStart(-1), nMessageStart(-1), vAddr(&vAddrIn), ban(0), doneAfter(0), nVersion(0), vSend(SER_NETWORK, MIN_PROTO_VERSION), vRecv(SER_NETWORK, MIN_PROTO_VERSION) {
...
//int main(void) {
//  CService ip("localhost", 44100, true);
//  vector<CAddress> vAddr;
//  vAddr.clear();
//  int clientV = 0;
//  str:string clientSV = "";
